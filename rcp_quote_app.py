...
st.caption("Voice-first • Consolidated & Sorted Quotes")
        ...
        if not current_text:
            ...
            for clause in clauses:
                ...
                if flared_match:
                    ...
                    st.session_state.items.append({
                        "type": "Flared End", "size": size, "qty": qty, 
...
if st.button("Generate Professional Quote", type="primary"):
    ...
    # Sort pipes from smallest to largest
    ...
    # Joint Lube
    if pipe_totals:
        total_pounds = sum(Decimal(lf) * Decimal(PIPE_WEIGHTS.get(size, 0)) 
                          for (size, cl, ton), lf in pipe_totals.items())
